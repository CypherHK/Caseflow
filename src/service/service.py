...
from agents.caseflow_events import running_event_for_graph_node, workflow_events_from_graph_update
        ...
        async with initialize_database() as saver, initialize_store() as store:
            ...
            for a in agents:
                ...
                if hasattr(agent, "builder"):
                    agent_name = getattr(agent, "name", None) or a.key
                    agent = agent.builder.compile(
                        checkpointer=saver,
                        store=store,
                        name=agent_name,
                    )
                    agent.name = agent_name
                    set_agent_graph(a.key, agent)
                else:
...
async def message_generator(
    ...
    agent: AgentGraph = get_agent(agent_id)
    kwargs, run_id = await _handle_input(user_input, agent)
    configurable = kwargs["config"].get("configurable", {})
    case_id = str(configurable.get("case_id") or "")
    emit_caseflow_events = agent_id == "caseflow-agent" and bool(
        configurable.get("demo_mode") or case_id
    )

    def workflow_event_sse(event: dict[str, Any]) -> str:
        return f"data: {json.dumps({'type': 'workflow_event', 'content': event})}\n\n"
            ...
            **kwargs, stream_mode=["updates", "messages", "custom"], subgraphs=True
            ...
            if stream_mode == "updates":
                for node, updates in event.items():
                    if emit_caseflow_events:
                        running_event = running_event_for_graph_node(
                            node,
                            run_id=str(run_id),
                            case_id=case_id,
                        )
                        if running_event:
                            yield workflow_event_sse(running_event)
                    ...
                    if node == "__interrupt__":
                        interrupt: Interrupt
                        if emit_caseflow_events:
                            for workflow_event in workflow_events_from_graph_update(
                                node,
                                updates,
                                run_id=str(run_id),
                                case_id=case_id,
                            ):
                                yield workflow_event_sse(workflow_event)
...
async def history(input: ChatHistoryInput, agent_id: str = DEFAULT_AGENT) -> ChatHistory:
    ...
    try:
        ...
        logger.error(f"Unknown agent requested for history: {agent_id}")
