...
class AgentClient:
    ...
    def _parse_stream_line(self, line: str) -> ChatMessage | str | dict[str, Any] | None:
        line = line.strip()
        if line.startswith("data: "):
            ...
            match parsed["type"]:
                ...
                case "workflow_event":
                    return {"type": "workflow_event", "content": parsed["content"]}
        return None
    ...
    def stream(
    ...
    ) -> Generator[ChatMessage | str | dict[str, Any], None, None]:
        """
        ...
        Returns:
            Generator[ChatMessage | str | dict[str, Any], None, None]: The response from the agent
        """
        if not self.agent:
            raise AgentClientError("No agent selected. Use update_agent() to select an agent.")
    ...
    async def astream(
        ...
        agent_config: dict[str, Any] | None = None,
        stream_tokens: bool = True,
    ) -> AsyncGenerator[ChatMessage | str | dict[str, Any], None]:
        """
        ...
        Returns:
            AsyncGenerator[ChatMessage | str | dict[str, Any], None]: The response from the agent
        """
        if not self.agent:
            raise AgentClientError("No agent selected. Use update_agent() to select an agent.")
        request = StreamInput(message=message, stream_tokens=stream_tokens)
    ...
    def get_history(self, thread_id: str) -> ChatHistory:
        ...
        history_path = f"/{self.agent}/history" if self.agent else "/history"
        try:
            response = httpx.post(
