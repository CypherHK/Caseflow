...
def test_get_history(agent_client):
    ...
    # Mock successful response
    mock_response = Response(
        200, json=HISTORY, request=Request("POST", "http://test/test-agent/history")
    )
    with patch("httpx.post", return_value=mock_response) as mock_post:
    ...
    # Test error response
    error_response = Response(
        500, text="Internal Server Error", request=Request("POST", "http://test/test-agent/history")
    )
    with patch("httpx.post", return_value=error_response):
        with pytest.raises(AgentClientError) as exc:
...
def test_get_history_without_agent_uses_legacy_route(mock_env):
    client = AgentClient(base_url="http://test", get_info=False)
    history = {"messages": [{"type": "ai", "content": "No agent selected."}]}
    mock_response = Response(200, json=history, request=Request("POST", "http://test/history"))

    with patch("httpx.post", return_value=mock_response) as mock_post:
        result = client.get_history("thread-legacy")

    assert result.messages[0].content == "No agent selected."
    assert mock_post.call_args.args[0] == "http://test/history"
