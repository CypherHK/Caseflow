...
def test_invoke_preserves_caseflow_custom_data(test_client, mock_agent) -> None:
    QUESTION = "客户问怎么重置账号密码"
    ANSWER = "CaseFlow 处理结果"
    CUSTOM_DATA = {
        "intent": "咨询类",
        "priority": "low",
        "evidence": ["FAQ evidence"],
        "proposed_action_plan": ["生成回复草稿"],
        "draft_response": "请使用重置密码入口。",
    ...
    mock_agent.ainvoke.return_value = [
        (
            ...
            {
                "messages": [
                    AIMessage(content=ANSWER, additional_kwargs={"custom_data": CUSTOM_DATA})
                ]
            },
        )
    ]

    response = test_client.post("/caseflow-agent/invoke", json={"message": QUESTION})
    assert response.status_code == 200

    output = ChatMessage.model_validate(response.json())
    assert output.type == "ai"
    assert output.content == ANSWER
    assert output.custom_data == CUSTOM_DATA
...
def test_history_custom_agent_uses_path_agent(test_client, mock_agent) -> None:
    CUSTOM_AGENT = "custom_agent"
    QUESTION = "CaseFlow history?"
    ANSWER = "Custom agent history."
    default_mock = AsyncMock()
    default_mock.aget_state.return_value = StateSnapshot(
        values={"messages": [HumanMessage(content="Default"), AIMessage(content="Default answer")]},
        next=(),
        config={},
        metadata=None,
        created_at=None,
    ...
    assert response.status_code == 200
    mock_agent.aget_state.assert_awaited_once()
...
def test_history_invalid_agent_does_not_fall_back_to_default(test_client) -> None:
    default_mock = AsyncMock()
    ...
    with patch("service.service.get_agent", side_effect=agent_lookup):
        response = test_client.post(
            "/missing-agent/history",
            json={"thread_id": "7bcc7cc1-99d7-4b1d-bdb5-e6f90ed44de6"},
        )

    assert response.status_code == 404
    assert response.json()["detail"] == "Agent not found: missing-agent"
    default_mock.aget_state.assert_not_awaited()
...
def test_info(test_client, mock_settings) -> None:
    ...
    assert output.default_agent == "caseflow-agent"
